    ...
    <script>
        ...
        $(document).ready(function() {
            $("#btn-edit").click(function() {
                let id = $(this).data("id")
                if ($("#pro_type1").val() === $("#pro_type1").val($(this).data("type"))) {
                    $("#pro_type1").prop("seleted", true);
                }
            ...
            $("#btn-del").click(function() {
                return confirm('ต้องการลบรายการนี้ใช่ไหม');
                let id = $(this).data("id")
                $.ajax({
        ...
        $(document).on("submit","#frm_product",function() {
        ...
        $(document).on("submit","#frm_sale",function() {
            let url = $(this).attr("action")
            let data = new FormData(this)
            $.ajax({
                url: url,
        ...
        $(document).on("submit","#frm_productEdit",function() {
            let url = $(this).attr("action")
            let data = new FormData(this)
            $.ajax({
                url: url,
        ...
        $(document).on("submit","#frm_productType",function() {
            let url = $(this).attr("action")
            let data = new FormData(this)
            $.ajax({
                url: url,
...
$(document).on("click","#btn-printBill",function() {
    ...
    $.ajax({
        ...
        success:function(res) {
            window.location.replace('bill.pdf');
        },error:function(err) {
            console.log(err);
        }
    })
...
$(document).on("click","#btn-conOrder",function() {
    let order_id = $(this).data("id");
    $.ajax({
